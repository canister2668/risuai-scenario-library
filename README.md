...
8. 고정 바 위의 **응답 후 지침 처리 (일회용)** 에서 AI 응답이 도착한 뒤 방금 넣은 지침 메시지를 어떻게 할지 고릅니다. 지침이 이후 요청마다 다시 전송되어 문맥을 차지하는 일을 막습니다.
   - `그대로 두기`(기본): 아무것도 하지 않습니다.
   - `삭제`: 지침 메시지를 채팅에서 지웁니다.
   - `[완료] 표시`: 본문은 남기고 앞에 `[완료]`만 붙입니다.
   - `한 줄로 줄이기`: `[지침 적용됨: 제목]` 한 줄만 남깁니다.

   응답이 오기 전에는 아무것도 바꾸지 않고, 다른 메시지와 채팅 상태(`scriptstate`)는 건드리지 않습니다. 처리 대기 목록은 저장되므로 새로고침 뒤에도 이어지며, 다른 채팅으로 옮겼거나 메시지를 직접 지운 경우에는 건너뜁니다.
9. 여러 상황극을 함께 넣으려면 상세에서 **담기**를 눌러 모읍니다. 헤더의 `◫` 배지로 **담은 상황극** 화면을 열어 순서를 바꾸거나 빼고, **인풋카드에 합치기**로 고른 순서대로 하나의 입력으로 만듭니다.
10. 인풋카드와 상세 본문에는 글자 수와 함께 **대략적인 토큰 추정치**가 표시됩니다. 한글은 영문보다 토큰을 훨씬 많이 쓰므로 글자 수만으로는 문맥 비용을 알기 어렵습니다. 플러그인에 토크나이저를 넣지 않는 대신 추정치임을 명시합니다.
11. 검색창이 비어 있으면 **최근 검색어** 5개가 칩으로 표시됩니다. 상세를 열었던 검색어만 기록되고, `기록 지우기`로 비울 수 있습니다.
...
- **보관함 백업**은 `전체 보관함 / 즐겨찾기만 / 현재 폴더만` 중에서 범위를 고릅니다. 범위를 좁혀도 폴더 골격은 함께 내보내므로 결과 JSON을 그대로 모듈로 가져올 수 있습니다.
...
이어서 데스크톱 1100px와 모바일 390px에서 로딩 중 즉시 닫기, 지연 본문 읽기, 더 보기/스크롤 자동 로드, 19금 필터 양방향, 즐겨찾기·최근, 검색, 최근 검색어 칩, 줄거리 상세, `{{char}}` 치환, 상세에서 바로 채팅에 추가, 인풋카드 추가, 토큰 추정 표시, 일회용 지침 삭제와 한 줄 축약, 담은 목록 순서 변경·합치기, 백업 범위 필터, 중복 클릭 방지, 동시 수정 거부와 가로 넘침을 확인합니다. Risu API는 테스트 더블이므로 실제 운영 사이트 설치와 실브라우저 통합 검증은 별도 단계입니다.
