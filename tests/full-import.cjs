...
(async()=>{
 ...
 try{
  ...
  assert((await page.locator('.result-count').innerText()).includes('527개 중 30개 표시'));
  assert.equal(await page.locator('.row-item').count(),30,'first page stays short on a phone');
  assert.equal(await page.evaluate(()=>JSON.parse(testHost.storage['scenario.v2.catalog']).lorebook.filter(x=>x.mode!=='folder').length),527);
  const cleanReport=await page.evaluate(()=>{const catalog=JSON.parse(testHost.storage['scenario.v2.catalog']),inputs=catalog.lorebook.filter(x=>x.mode!=='folder').map(x=>ScenarioCore.cleanInput(testHost.storage['scenario.v2.content.'+x.id]));return{
  ...
  // Every folder carries its own count so 527 entries stay navigable.
  assert.equal(await page.getByRole('button',{name:'성인 271'}).count(),1);
  await page.getByRole('button',{name:'성인 271'}).click();
  await page.waitForFunction(()=>document.querySelector('.result-count')?.textContent.includes('271개'));
  ...
  console.log(JSON.stringify({seeded:527,firstPage:30,adult:271,mobileOverflow:false},null,2));
